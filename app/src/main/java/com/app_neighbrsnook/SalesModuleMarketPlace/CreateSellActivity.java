...
public class CreateSellActivity extends AppCompatActivity implements MarketPlaceCategoryAdapter.CategoryInterface, View.OnClickListener, ImageUploadAdapter1.ImageItemClick, ImageUploadAdapter.ImageRequest, MediaViewAdapter.OnItemClickListener {
    ...
    int totalImageCount, totalVideoCount, postVideoSize;
    ...
    private void compressVideo(Uri videoUri) {
        ...
        VideoCompressor.compressVideo(this, videoUri, new VideoCompressor.CompressionListener() {
            @Override
            public void onCompressionSuccess(Uri compressedUri) {
                runOnUiThread(() -> {
                    progressDialog.dismiss();
                    videoUris.add(compressedUri);

                    // Add to file paths for cleanup
                    String compressedPath = getPath(CreateSellActivity.this, compressedUri);
                    if (compressedPath != null) {
                        videoFilePaths.add(compressedPath);
                    }
            ...
            @Override
            public void onCompressionFailure(String error) {
                runOnUiThread(() -> {
                    progressDialog.dismiss();
                    Toast.makeText(CreateSellActivity.this, "Video compression failed: " + error, Toast.LENGTH_SHORT).show();
                    Log.e("VideoCompression", "Compression failed: " + error);
                });
            }

            @Override
            public void onProgressUpdate(int progress) {
                // You can update progress dialog if needed
                runOnUiThread(() -> {
                    if (progressDialog != null && progressDialog.isShowing()) {
                        progressDialog.setMessage("Compressing video... " + progress + "%");
                    }
                });
            }
        });
    ...
    private boolean CheckAllFields() {
        if (item_name.getText().toString().matches("")) {
            ...
            return false;
        } else if (price_status.matches("")) {
            Toast.makeText(CreateSellActivity.this, "Please select type", Toast.LENGTH_SHORT).show();
            return false;
