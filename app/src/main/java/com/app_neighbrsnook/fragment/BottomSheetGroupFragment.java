...
public class BottomSheetGroupFragment extends BottomSheetDialogFragment {
    ...
    public BottomSheetGroupFragment(Context context, List<Listdatum> mList, int pos) {
    ...
    public void setupDialog(Dialog dialog, int style) {
        View contentView = View.inflate(getContext(), R.layout.fragment_bottom_sheet_group, null);
        ...
        if (mList.get(pos).getFavouritstatus() == 0) {
            fav_tv.setText("Favourite Group");
            fav_subtitle_tv.setText("Add this to my favourites.");
        } else {
            fav_tv.setText("Unfavourite Group");
                                ...
                                public void onResponse(Call<CommonPojoSuccess> call, Response<CommonPojoSuccess> response) {
                                    if (response.isSuccessful() && response.body() != null) {
                                        ...
                                        if ("success".equals(commonPojoSuccess.getStatus())) {
                                            Intent i = new Intent(BottomSheetGroupFragment.this.context, MainActivity.class);
        ...
        share_rl.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String imageUrl = mList.get(pos).getGroupImage();
    ...
    private void shareImage(File imageFile, int pos) {
        try {
            ...
            shareIntent.putExtra(Intent.EXTRA_SUBJECT, "Neighbrsnook Group");
            shareIntent.putExtra(Intent.EXTRA_TEXT, "Checkout this group on Neighbrsnook,\n" + "\n" + mList.get(pos).getGroup_name() +
                    "\n" + mList.get(pos).getGroup_description() +
