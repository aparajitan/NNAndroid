...
public class EventLikesMemberListAdapter extends RecyclerView.Adapter<EventLikesMemberListAdapter.BankViewHolder> {
    private final List<EventUsersLikesEvent> groupListPojos;
    Dialog newRequest;
    Context mcon;
    int id;


    public EventLikesMemberListAdapter(List<EventUsersLikesEvent> lists, Dialog newRequest) {
    ...
    @NonNull
    @Override
    public EventLikesMemberListAdapter.BankViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new EventLikesMemberListAdapter.BankViewHolder(view);
    }
    @Override
    public void onBindViewHolder(@NonNull EventLikesMemberListAdapter.BankViewHolder holder, int position) {
        EventUsersLikesEvent eventjoinuserlist = groupListPojos.get(position);
        ...
        if (eventjoinuserlist.getUserpic().isEmpty()) {
            holder.img_event_userp_pic.setImageResource(R.drawable.profile);
        } else{
            Picasso.get().load(eventjoinuserlist.getUserpic()).fit().centerCrop().error(R.drawable.profile).placeholder(R.drawable.profile)
    ...
    public int getItemCount() {
        return (groupListPojos != null) ? groupListPojos.size() : 0;
    }
