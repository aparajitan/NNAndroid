...
public class MarketPlaceDetailImgAdapter extends RecyclerView.Adapter<MarketPlaceDetailImgAdapter.MyViewHolder> {
    List<ImagePojo> dataList;
    ImgRequest imgRequest;
    Context context;


    public MarketPlaceDetailImgAdapter(List<ImagePojo>  dataList, ImgRequest imgRequest ) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
       ...
       // holder.editDetails.setVisibility(View.GONE);
        ...
        Picasso.get().load(dataList.get(position).getImg()).placeholder(R.drawable.marketplace_white_background)
                .into(holder.imageView);
       /* if (dataList.get(0).getImg().size()){
