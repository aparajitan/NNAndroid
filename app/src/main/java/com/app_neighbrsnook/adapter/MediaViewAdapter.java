...
public class MediaViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    static class VideoViewHolder extends RecyclerView.ViewHolder {
        ...
        VideoViewHolder(@NonNull View itemView) {
            ...
            // Video completion listener add karo
        ...
        void bind(Uri uri, OnItemClickListener listener, int position) {
            ...
            // Video completion listener set karo har naye video ke liye
            setupVideoCompletionListener();
        }

        private void setupVideoCompletionListener() {
            videoView.setOnCompletionListener(mp -> {
                // Video end hone par UI reset karo
                runOnUiThread(() -> {
                    isPlaying = false;
                    resetVideoUI();
                    videoView.seekTo(0); // Video start position par reset karo
                });
            });
        }
        ...
        // Helper method for runOnUiThread
        private void runOnUiThread(Runnable action) {
            if (itemView.getHandler() != null) {
                itemView.getHandler().post(action);
            }
        }
    }
}